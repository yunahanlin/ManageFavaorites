var vm = new Vue({
	el:".container",
	data:{
        filterAddress:[],		//把获取到的jason地址列表储存于此。
	...
	computed:{
		...
		addressLimitBy:function(){
			return this.filterAddress.slice(0,this.moreAddressNumber);
		}
	},
	filters:{
	},
	methods:{
		getAddressList:function(){
			...
			this.$http.get('../data/address.json',{'id':123}).then(function(res){
				console.log(res)
				// alert('5555')
				_this.filterAddress = res.data.result;
			});
		},
        loadMoreAddress:function(){
			if(this.moreAddressNumber === 3){
				this.moreAddressNumber = this.filterAddress.length;
			}else {
				this.moreAddressNumber = 3;
			}
		},
        delFlag:function(index){
		...
		setDefault:function(addressId){
			...
			//一致的元素就是我们点击的那个元素。
			this.filterAddress.forEach(function(item,addressListIndex){
